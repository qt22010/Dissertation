...
X_train_temp, X_test_temp, y_train_temp, y_test_temp = train_test_split(
    sequences_temp, target_values_temp, test_size=0.2, shuffle=False
)
X_train_rain, X_test_rain, y_train_rain, y_test_rain = train_test_split(
    sequences_rain, target_values_rain, test_size=0.2, shuffle=False
)

# Build the LSTM model for temperature using Functional API
input_temp = Input(shape=(input_sequence_length, 1))
lstm_temp = LSTM(units=64, activation='relu')(input_temp)
output_temp = Dense(1)(lstm_temp)
model_temp = Model(inputs=input_temp, outputs=output_temp)
model_temp.compile(optimizer='adam', loss='mean_squared_error')
model_temp.fit(X_train_temp, y_train_temp, epochs=50, batch_size=32, validation_data=(X_test_temp, y_test_temp))

# Build the LSTM model for rain using Functional API
input_rain = Input(shape=(input_sequence_length, 1))
lstm_rain = LSTM(units=64, activation='relu')(input_rain)
output_rain = Dense(1)(lstm_rain)
model_rain = Model(inputs=input_rain, outputs=output_rain)
model_rain.compile(optimizer='adam', loss='mean_squared_error')
model_rain.fit(X_train_rain, y_train_rain, epochs=50, batch_size=32, validation_data=(X_test_rain, y_test_rain))
...
for _ in range(7):
    prediction_temp = model_temp.predict(input_sequence_temp)
    prediction_rain = model_rain.predict(input_sequence_rain)
