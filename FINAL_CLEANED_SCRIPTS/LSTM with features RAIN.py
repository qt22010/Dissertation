...
data = pd.read_csv('rain_weather.csv')
...
def build_model(units, learning_rate, dropout_rate, num_lstm_layers,num_nodes_per_layer):
    model = Sequential()
    """
    a linear stack of layers, adding layers to this model one by one
    """
    # Input LSTM layer
...
"""
build_model is responsible for constructing a LSTM for a regression task, 
specifically for predicting a sequence of temperature values.
...
"""
sets up a regression model for use with 
scikit machine learning pipelines and functions.
"""
...
early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True) # patience detemines consectuve epochs 
#the train can continue without improvement

"""
technique used during the training of machine learning models to prevent overfitting 
and to stop training when a certain condition is met 
"""
...
plt.plot(subset_y_test[:, 0], label='Real Precipitation', color='orange')
...
plt.title('Predicted vs. Real Precipitaion (Latest 5 Years)')
